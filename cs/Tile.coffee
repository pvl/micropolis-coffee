    ...
    class Tile
        ...
        constructor: (tileValue, bitMask) ->
            ...
            if arguments.length and
                (typeof(tileValue) != 'number' or
                 (arguments.length > 1 and typeof(bitMask) != 'number') or
                 tileValue < 0)
                throw new Error 'Invalid parameter'

            if arguments.length > 1 and (tileValue >= Tile.TILE_COUNT or bitMask < Tile.BIT_START or bitMask >= (Tile.BIT_END << 1))
                throw new Error 'Invalid parameter'

            @_value = tileValue
            #If no value supplied, default to Tile.DIRT
            @_value = Tile.DIRT if @_value == undefined
            @_value |= bitMask if arguments.length > 1

        getValue: ->
            @_value & Tile.BIT_MASK

        setValue: (tileValue) ->
            if not arguments.length or typeof(tileValue) != 'number' or tileValue < 0
                throw new Error 'Invalid parameter'
            existingFlags = 0
            if tileValue < Tile.BIT_START
                existingFlags = @getFlags()
            @_value = tileValue | existingFlags

        isBulldozable: () ->
            (@_value & Tile.BULLBIT) > 0

        isAnimated: () ->
            (@_value & Tile.ANIMBIT) > 0

        isConductive: () ->
            (@_value & Tile.CONDBIT) > 0

        isCombustible: () ->
            (@_value & Tile.BURNBIT) > 0

        isPowered: () ->
            (@_value & Tile.POWERBIT) > 0

        isZone: () ->
            (@_value & Tile.ZONEBIT) > 0

        addFlags: (bitMask) ->
            if not arguments.length or typeof(bitMask) != 'number' or bitMask < Tile.BIT_START or bitMask >= (Tile.BIT_END << 1)
                throw new Error 'Invalid parameter'
            @_value |= bitMask
        ...
        setFlags: (bitMask) ->
            if typeof(bitMask) != 'number' or bitMask < Tile.BIT_START or bitMask >= (Tile.BIT_END << 1)
                throw new Error 'Invalid parameter'
            existingValue = @_value & ~Tile.ALLBITS
            @_value = existingValue | bitMask
        ...
        set: (tileValue, bitMask) ->
            if arguments.length < 2 or typeof(tileValue) != 'number' or typeof(bitMask) != 'number' or tileValue >= Tile.TILE_COUNT
                throw new Error 'Invalid parameter'
